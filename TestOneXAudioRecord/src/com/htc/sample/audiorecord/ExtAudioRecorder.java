...
public class ExtAudioRecorder implements Runnable {
	...
	private static String mp3Path = Environment.getExternalStorageDirectory()
			+ "/FOLDER/tmp/tmp.mp3";
	...
	public static ExtAudioRecorder getInstanse(Boolean recordingCompressed) {
		...
		if (recordingCompressed) {
			...
			lastSampleRate = sampleRates[3];
		} else {
			int i = 0;
			do {

				lastSampleRate = sampleRates[i];
        ...
        Log.i("***TestOneXAudioRecordActivity***", "last sample rate tried = " + lastSampleRate);

        Log.i("***TestOneXAudioRecordActivity***", "ExtAudioRecorder.State.INITIALIZING = " + (result.getState() == ExtAudioRecorder.State.INITIALIZING));
			...
			if (state == State.INITIALIZING) {
				if (rUncompressed) {
					if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED)
						...
						randomAccessWriter = new RandomAccessFile(FullFilePath,
								"rw");
